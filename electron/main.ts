...
app.whenReady().then(async () => {
  ...
  ipcMain.handle('personnel:list', async (_event, params = {}) => { const page = Math.max(1, Number(params.page) || 1); const pageSize = Math.min(100, Math.max(5, Number(params.pageSize) || 20)); const search = String(params.search || '').trim(); const rank = String(params.kategoriPangkat || '').trim(); const where: any = {}; const conditions: any[] = []; if (search) conditions.push({ OR: [{ nrp: { contains: search } }, { nama: { contains: search } }] }); if (rank) conditions.push({ OR: [{ pangkat: { contains: rank } }, { kategoriPangkat: rank }] }); if (conditions.length) where.AND = conditions; const [items, total] = await Promise.all([prisma.personnel.findMany({ where, orderBy: { nama: 'asc' }, skip: (page - 1) * pageSize, take: pageSize }), prisma.personnel.count({ where })]); return { items, total, page, pageSize, totalPages: Math.max(1, Math.ceil(total / pageSize)) } })
  ...
  ipcMain.handle('report:generate', async (_event, input) => { const result = await generateReport(prisma, String(input.type || ''), input); await logActivity(prisma, { type: 'SUCCESS', title: 'Generate Report', description: `${result.fileName} berhasil dibuat`, performedBy: 'Operator' }); return result })
